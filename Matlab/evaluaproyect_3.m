function [A,B]=evaluaproyect_3(d)
%Este programa utiliza la varianza del valor actual neto en el criterio 
%de parada haciendo uso del TIR como variable de control para reducir 
%la varianza y reducir así el numero de iteraciones.
%A: Valor Actual Neto.
...
if iter==1
   ...
   var_tir(iter)=0;
    ...
    var_tir(iter)=(1-1/(iter-1))*var_tir(iter-1)+iter*(m_tir(iter)-m_tir(iter-1))^2;
end

if iter>30
    cova=((van*tasa')/iter)-(m_van(iter)*m_tir(iter));
    sk=sqrt((var_van(iter)-(((cova)^2)/var_tir(iter)))/iter);
end
iter=iter+1;
end
